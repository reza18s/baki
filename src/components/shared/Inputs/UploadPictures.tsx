...
export default function UploadPictures({
  ...
  >([null, null, null]);
  const {
    userInfo: { mainImage, images, avatar },
    updateUserInfo,
  } = useLocalStore((s) => s);
  ...
  return (
    <div className="mx-auto flex w-full flex-col items-center justify-center gap-2">
      <div className="flex w-full gap-2">
        <div
          className={cn(
            'relative aspect-square w-full flex-[2] rounded-2xl bg-gray-100',
          )}
        >
        ...
        {onChange && (
          <div className="flex flex-1 flex-col gap-2">
            <div
              onClick={() => hs.push(paths.profile.record)}
              className="relative flex aspect-square w-full items-center justify-center rounded-2xl bg-gray-100"
            >
              <IcMicrophone className="size-10"></IcMicrophone>
            </div>
            <div
              className="relative flex aspect-square w-full items-center justify-center rounded-2xl bg-gray-100"
              onClick={() => hs.push(paths.profile.avatar)}
            >
              <img
                src={
                  allAvatars.find((a) => a.path === avatar)?.avatar ||
                  allAvatars[0].avatar
                }
                alt="CardAvatar"
