...
export default function GetSpecialty(props: {
  ...
  return (
    <div className="flex flex-col gap-y-[40px] w-full h-full min-h-fit pb-20">
            ...
            <div className="flex flex-wrap gap-3">
              {item.subItems.map((subItem) => (
                <button
                  ...
                  key={subItem}
                  className={`rounded-[32px] bg-[#F1F5F9] p-[12px] pl-[16px] max-w-fit flex items-center gap-x-3 ${selectedInterests.includes(subItem) ? "bg-brand-yellow" : "bg-[#F1F5F9]"}`}
                >
                  <p className="text-[#1a1d1e] text-sm">{subItem}</p>
                </button>
              ))}
            </div>
          </div>
      ...
      {/* Footer */}
      <div
        className={`bottom-0 flex items-center justify-end gap-x-[16px] min-w-fit w-[90%] px-2 bg-white py-3 sticky`}
      >
        <button
