    ...
    <div className="flex flex-col gap-y-[40px] w-full h-full min-h-fit pb-20">
      ...
      <div className="flex flex-col items-start justify-around gap-y-3 min-h-fit w-full">
        {interestsItems.map((item, index) => {
          const nextIndex =
            interestsItems.length + 1 >= index && (showAll || index < 9)
              ? index + 1
              : 0;
              ...
              <div className="flex items-center gap-x-2">
                {(showAll || index < 9) && (
                  <button
                    ...
                    key={index}
                    className={`rounded-[32px] p-[12px] max-w-fit flex items-center gap-x-3 ${selectedInterests.includes(item.title) ? "bg-brand-yellow" : "bg-[#F1F5F9]"}`}
                  >
                    <img src={item.icon} alt={item.title} className="w-5 h-5" />
                    <p className="text-[#1a1d1e] text-sm">{item.title}</p>
                  </button>
                )}
                {nextIndex > 0 && nextItem && (
                  <button
                    ...
                    key={nextIndex}
                    className={`rounded-[32px] bg-[#F1F5F9] p-[12px] max-w-fit flex items-center gap-x-3 ${selectedInterests.includes(nextItem.title) ? "bg-brand-yellow" : "bg-[#F1F5F9]"}`}
                  >
                    <img
                      src={nextItem.icon}
                      alt={nextItem.title}
                      className="w-5 h-5"
                    />
                    <p className="text-[#1a1d1e] text-sm">{nextItem.title}</p>
                  </button>
                )}
              </div>
            );
        ...
        <button
          ...
          className={`${showAll ? "hidden" : "flex"} items-center justify-center gap-x-2 w-full -mt-16`}
        >
          <SolarIconSet.AltArrowDown size={16} />
          <p className="text-sm text-[#1a1d1e] font-bold">مشاهده همه</p>
        </button>
      </div>
      {/* Footer */}
      <div
        className={`bottom-0 flex items-center justify-between gap-x-[16px] min-w-fit w-[90%] px-2 bg-white py-3 ${showAll ? "sticky" : "absolute"}`}
      >
