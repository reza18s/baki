...
export default function Login() {
    ...
    <div className="text-black p-[24px] relative h-full" dir="rtl">
      ...
      <div className="flex items-center justify-between gap-x-[7px] w-full">
        ...
        <div className="flex justify-around gap-x-[1.81px] w-full">
          {/* Active Step */}
          <div
            className={`w-[143.09px] h-[3.62px] ${
              step === 0 ? 'bg-[#ffcc4e]' : 'bg-slate-100'
            } rounded-xl`}
          />
          {/* Inactive Step */}
          <div
            className={`w-[143.09px] h-[3.62px] ${
              step === 1 ? 'bg-[#ffcc4e]' : 'bg-slate-100'
            } rounded-xl`}
          />
        </div>
      </div>
        ...
        <div className="flex flex-col gap-y-[40px]">
          <div className="flex flex-col gap-y-[16px]">
            ...
            <p className="text-sm font-medium font-['IRANSansXFaNum'] leading-tight text-[#64748B]">
              ما با اطمینان از واقعی بودن همه افرادی که در باکی هستند از کاربران
              خود محافظت می کنیم.
            </p>
          </div>
 ...
 font-medium leading-none font-['IRANSansX']"
        ...
        <button
          disabled={watch('phoneNumber')?.length !== 11}
          onClick={handleSignup}
          className={`px-[20px] py-[16px] ${
            watch('phoneNumber')?.length === 11
              ? 'bg-[#ffcc4e]'
              : 'bg-slate-100'
          } rounded-[12px] text-slate-400 font-bold font-['IRANSansXFaNum'] leading-none`}
        >
          بعدی
        </button>
      </div>
